...
export class NetworkClient {
  ...
  constructor() {
    this.firebaseSync = new FirebaseSync({
      ...
      onVoiceJoin: (data) => {
        this.voiceEventListeners.get('voice:userJoined')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice:user-joined')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice_user_joined')?.forEach((cb) => cb(data));
        this.onVoiceUserJoined?.(data);
      },
      onVoiceLeave: (data) => {
        this.voiceEventListeners.get('voice:userLeft')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice:user-left')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice_user_left')?.forEach((cb) => cb(data));
        this.onVoiceUserLeft?.(data);
      },
      onVoiceSignal: (data) => {
        this.voiceEventListeners.get('voice:signal')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice_signal')?.forEach((cb) => cb(data));
        this.onVoiceSignal?.(data);
      },
      onVoiceSpeaking: (data) => {
        this.voiceEventListeners.get('voice:speaking')?.forEach((cb) => cb(data));
        this.voiceEventListeners.get('voice_speaking')?.forEach((cb) => cb(data));
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      // Purge only deprecated mock worker references
      const storedWorker = localStorage.getItem('aether_cf_worker_url');
      if (storedWorker && storedWorker.includes('dry-snow-f534')) {
  ...
  public isCloudflareStaticHost(): boolean {
    ...
    return h.includes('pages.dev') || h.includes('github.io') || h.includes('workers.dev');
  }
  ...
  public async createRoom(playerName: string, preferredRole?: PlayerRole, stageId: number = 1): Promise<void> {
    ...
    if (this.networkMode === 'websocket' || this.networkMode === 'auto') {
      ...
      if (this.networkMode === 'websocket') {
        throw new Error('اتصال به سرور وب‌سوکت ناموفق بود.');
      }
    }

    // 2. If explicit P2P mode
    if (this.networkMode === 'p2p') {
      await this.createRoomP2P(playerName, role);
      return;
    }

    // 3. Primary in Auto & Firebase Mode: Firebase Firestore Cloud Database
    if (this.networkMode === 'firebase' || this.networkMode === 'auto') {
      try {
        ...
        console.warn('Firebase room creation note, falling back to P2P WebRTC:', fbErr?.message || fbErr);
    ...
    // 4. Ultra-fast P2P WebRTC (zero cloud quotas, direct connection)
    try {
      await this.createRoomP2P(playerName, role);
      return;
    } catch (p2pErr: any) {
      console.warn('P2P creation fallback failed:', p2pErr);
      throw new Error('امکان ساخت اتاق با پروتکل‌های موجود فراهم نشد. لطفاً اتصال اینترنت خود را بررسی کنید.');
    }
  }
  ...
  public async joinRoom(code: string, playerName: string, preferredRole?: PlayerRole): Promise<void> {
    ...
    if (this.networkMode === 'firebase' || this.networkMode === 'auto') {
      try {
        ...
        console.warn('Firebase joinRoom note, checking fallback transports:', fbErr?.message || fbErr);
        if (this.networkMode === 'firebase') {
          throw fbErr;
        }
      }
    ...
    return {
      ...
      emit: (event: string, data?: any) => {
        if (event === 'voice:join' || event === 'voice_join') {
          ...
          } else if (this.activeTransport === 'firebase' || this.firebaseSync) {
            this.firebaseSync.sendVoiceJoin();
          }
          ...
          if (this.activeTransport === 'ws' && this.ws?.readyState === WebSocket.OPEN) {
            ...
            this.firebaseSync.sendVoiceLeave();
          }
          ...
          if (this.activeTransport === 'ws' && this.ws?.readyState === WebSocket.OPEN) {
            ...
            this.firebaseSync.sendVoiceSignal(data?.signal, data?.type || data?.signalType, data?.to);
          }
          ...
          if (this.activeTransport === 'ws' && this.ws?.readyState === WebSocket.OPEN) {
            ...
            this.firebaseSync.sendVoiceSpeaking(!!data?.isSpeaking);
          }
